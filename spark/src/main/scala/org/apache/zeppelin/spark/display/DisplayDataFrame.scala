 ...
 */
class DisplayDataFrame(val sc: SparkContext) extends DisplayFunction with AbstractDisplay {

  val logger:Logger = LoggerFactory.getLogger(classOf[DisplayDataFrame])

  val cls = try {
    Class.forName("org.apache.spark.sql.DataFrame")
  } catch {
    case cnfe: ClassNotFoundException =>
      throw new InterpreterException("Cannot instantiate 'org.apache.spark.sql.DataFrame'. " +
        "Are you sure you have Spark 1.3 or above ?")
  ...
  override def display(anyObject: Any, displayParams: DisplayParams): Unit = {
    ...
    require(anyObject != null, "Cannot display null DataFrame")
