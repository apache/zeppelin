...
/**
 * Display function for RDD of Product
 */
class DisplayRDD extends DisplayFunction with AbstractDisplay {

  val logger:Logger = LoggerFactory.getLogger(classOf[DisplayRDD])

  override def canDisplay(anyObject: Any): Boolean = {
    anyObject != null && classOf[RDD[Product]].isAssignableFrom(anyObject.getClass)
  }
  ...
  override def display(anyObject: Any, displayParams: DisplayParams): Unit = {
    ...
    require(anyObject != null, "Cannot display null RDD")
    val rdd: RDD[_] = anyObject.asInstanceOf[RDD[_]]
    ...
    val output = printFormattedData(rdd.take(newParams.maxResult), newParams.columnsLabel.asScala.toArray: _*)
