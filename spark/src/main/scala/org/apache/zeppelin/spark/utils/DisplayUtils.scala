...
object DisplayUtils {
  ...
  implicit def toDisplayRDDFunctions[T <: Product](rdd: RDD[T]): DisplayRDDFunctions[T] = {
    new DisplayRDDFunctions[T](rdd)
  }

  implicit def toDisplayTraversableFunctions[T <: Product](traversable: Traversable[T])
  : DisplayTraversableFunctions[T] = new DisplayTraversableFunctions[T](traversable)

  def html(htmlContent: String = ""): String = s"%html $htmlContent"

  def img64(base64Content: String = ""): String = s"%img $base64Content"

  def img(url: String): String = s"<img src='$url' />"
}
...
class DisplayTraversableFunctions[T <: Product](val traversable: Traversable[T]
                                               ) extends DisplayCollection[T] {
