...
```bash
./mvnw clean package -DskipTests -Pspark-3.5 -Pflink-1.20
```
...
`-Pflink-1.20` tells maven to build a Zeppelin with Flink 1.20.
...
```bash
wget -O flink-1.20.4-bin-scala_2.12.tgz "https://www.apache.org/dyn/closer.lua/flink/flink-1.20.4/flink-1.20.4-bin-scala_2.12.tgz?action=download"
tar -xzvf flink-1.20.4-bin-scala_2.12.tgz
```

This will download Flink 1.20.4.

Start the Flink Cluster.

```bash
flink-1.20.4/bin/start-cluster.sh
```
...
Return to the directory where you have been downloading, this tutorial assumes that is `$HOME`. Clone Flink,  check out release-1.20.4, and build.

```bash
cd $HOME
git clone https://github.com/apache/flink.git
cd flink
git checkout release-1.20.4
...
```bash
flink-1.20.4/bin/stop-cluster.sh
flink-1.20.4/bin/start-cluster.sh
```
