...
class BoundValuesParser extends RegexParsers with JavaTokenParsers {
  ...
  def standardDate: Parser[String] = {
    s"""'${STANDARD_DATE_PATTERN.toString}(?:\\.\\d{3})?'""".r ^^ {
      _.replaceAll("'", "")
    }
  }
  ...
  def list: Parser[String] = "[" ~> repsep(value, ",") <~ "]" ^^ {
    _.mkString("[", ",", "]")
  }

  def set: Parser[String] = "{" ~> repsep(value, ",") <~ "}" ^^ {
    _.mkString("{", ",", "}")
  }

  def map: Parser[String] = "{" ~> repsep(member, ",") <~ "}" ^^ {
    _.mkString("{", ", ", "}")
  }

  def tuple: Parser[String] = "(" ~> repsep(value, ",") <~ ")" ^^ {
    _.mkString("(", ",", ")")
  }

  def udt: Parser[String] = "{" ~> repsep(udtMember, ",") <~ "}" ^^ {
    _.mkString("{", ", ", "}")
  }

  def member: Parser[String] = {
    quotedString ~ ":" ~ value ^^ { case name ~ sep ~ mapVal => name + ": " + mapVal }
  }

  def udtColumnName: Parser[String] = """(?:(?:[a-zA-Z][a-zA-Z0-9_]*)|(?:"[^"]+"))""".r

  def udtMember: Parser[String] = {
    udtColumnName ~ ":" ~ value ^^ { case name ~ sep ~ mapVal => name + ": " + mapVal }
  }

  def zeppelinVariable: Parser[String] = {
    "{{" ~"""\w+=[^}]+""".r ~ "}}" ^^ {
      case prefix ~ variable ~ suffix => prefix + variable + suffix
    }
  }
