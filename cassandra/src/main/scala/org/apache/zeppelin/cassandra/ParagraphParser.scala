...
object ParagraphParser {
   ...
   */
  val UDF_PATTERN =
    """(?is)\s*(CREATE(?:\s+OR REPLACE)?\s+FUNCTION(?:\s+IF\s+NOT\s+EXISTS)
      |?.+?(?:\s+|\n|\r|\f)AS(?:\s+|\n|\r|\f)(?:'|\$\$).+?(?:'|\$\$)\s*;)""".r
  ...
  val DESCRIBE_MATERIALIZED_VIEW_PATTERN = {
    ("""^(?i)\s*(?:DESCRIBE|DESC)\s+MATERIALIZED\s+VIEW\s*(""" + VALID_IDENTIFIER + """);\s*$""").r
  }
...
class ParagraphParser extends RegexParsers {
  ...
  def singleLineCommentDoubleSlashes: Parser[Comment] = {
    """\s*//.*""".r ^^ { case text => Comment(text.trim.replaceAll("//", "")) }
  }
  ...
  def multiLineComment: Parser[Comment] = {
    """(?s)/\*(.*)\*/""".r ^^ { case text => Comment(text.trim.replaceAll("""/\*""", "").replaceAll("""\*/""", "")) }
  }

  //Query parameters
  def consistency: Parser[Consistency] =
    """\s*@consistency.+""".r ^^ { case x => extractConsistency(x.trim) }

  def serialConsistency: Parser[SerialConsistency] = {
    """\s*@serialConsistency.+""".r ^^ { case x => extractSerialConsistency(x.trim) }
  }
  ...
  def genericStatement: Parser[SimpleStm] = {
    s"""$GENERIC_STATEMENT_PREFIX[^;]+;""".r ^^ { case x => extractSimpleStatement(x.trim) }
  }
  ...
  def removePrepare: Parser[RemovePrepareStm] = {
    """\s*@remove_prepare.+""".r ^^ { case x => extractRemovePreparedStatement(x.trim) }
  }
  ...
  private def describeCluster: Parser[DescribeClusterCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+CLUSTER.*""".r ^^ {
      extractDescribeClusterCmd(_)
    }

  private def describeKeyspaces: Parser[DescribeKeyspacesCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+KEYSPACES.*""".r ^^ {
      extractDescribeKeyspacesCmd(_)
    }

  private def describeTables: Parser[DescribeTablesCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+TABLES.*""".r ^^ {
      extractDescribeTablesCmd(_)
    }

  private def describeTypes: Parser[DescribeTypesCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+TYPES.*""".r ^^ {
      extractDescribeTypesCmd(_)
    }

  private def describeFunctions: Parser[DescribeFunctionsCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+FUNCTIONS.*""".r ^^ {
      extractDescribeFunctionsCmd(_)
    }

  private def describeAggregates: Parser[DescribeAggregatesCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+AGGREGATES.*""".r ^^ {
      extractDescribeAggregatesCmd(_)
    }

  private def describeMaterializedViews: Parser[DescribeMaterializedViewsCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+MATERIALIZED\s+VIEWS.*""".r ^^ {
      extractDescribeMaterializedViewsCmd(_)
    }

  private def describeKeyspace: Parser[DescribeKeyspaceCmd] =
    """\s*(?i)(?:DESCRIBE|DESC)\s+KEYSPACE\s+.+""".r ^^ {
      extractDescribeKeyspaceCmd(_)
    }

  private def describeTable: Parser[DescribeTableCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+TABLE\s+.+""".r ^^ {
      extractDescribeTableCmd(_)
    }

  private def describeType: Parser[DescribeTypeCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+TYPE\s+.*""".r ^^ {
      extractDescribeTypeCmd(_)
    }

  private def describeFunction: Parser[DescribeFunctionCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+FUNCTION\s+.*""".r ^^ {
      extractDescribeFunctionCmd(_)
    }

  private def describeAggregate: Parser[DescribeAggregateCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+AGGREGATE\s+.*""".r ^^ {
      extractDescribeAggregateCmd(_)
    }

  private def describeMaterializedView: Parser[DescribeMaterializedViewCmd] =
    """(?i)\s*(?:DESCRIBE|DESC)\s+MATERIALIZED\s+VIEW\s+.*""".r ^^ {
      extractDescribeMaterializedViewCmd(_)
    }


  //Help
  private def helpCommand: Parser[HelpCmd] =
    """(?i)\s*HELP.*""".r ^^ {
      extractHelpCmd(_)
    }
  ...
  private def insert: Parser[SimpleStm] =
    """(?i)INSERT [^;]+;""".r ^^ {
      SimpleStm(_)
    }

  private def update: Parser[SimpleStm] =
    """(?i)UPDATE [^;]+;""".r ^^ {
      SimpleStm(_)
    }

  private def delete: Parser[SimpleStm] =
    """(?i)DELETE [^;]+;""".r ^^ {
      SimpleStm(_)
    }
  ...
  def extractSimpleStatement(text: String): SimpleStm = {
    text match {
      ...
      case _ => throw new InterpreterException(s"Invalid statement '$text'." +
        s" Did you forget to add ; (semi-colon) at the end of each CQL statement ?")
    }
  }
  ...
  def extractUdfStatement(text: String): SimpleStm = {
    text match {
      ...
      case _ => throw new InterpreterException(s"Invalid statement '$text' " +
        s"for UDF creation. Did you forget to add ; (semi-colon) at the end of each CQL statement ?")
    }
  }
  ...
  def extractDescribeTableCmd(text: String): DescribeTableCmd = {
    text match {
      ...
      case _ => throw new InterpreterException(s"Invalid syntax for DESCRIBE TABLE. " +
        s"""It should comply to the patterns: ${DESCRIBE_TABLE_WITH_KEYSPACE_PATTERN.toString}
            |or ${DESCRIBE_TABLE_PATTERN.toString}""".stripMargin)
    }
  }
  ...
  def extractDescribeTypeCmd(text: String): DescribeTypeCmd = {
    text match {
      ...
      case _ => throw new InterpreterException(s"Invalid syntax for DESCRIBE TYPE. " +
        s"""It should comply to the patterns: ${DESCRIBE_TYPE_WITH_KEYSPACE_PATTERN.toString}
            |or ${DESCRIBE_TYPE_PATTERN.toString}""".stripMargin)
    }
  }
  ...
  def extractDescribeFunctionCmd(text: String): DescribeFunctionCmd = {
    text match {
      case DESCRIBE_FUNCTION_WITH_KEYSPACE_PATTERN(keyspace, function) =>
        new DescribeFunctionCmd(Option(keyspace), function)
      case DESCRIBE_FUNCTION_PATTERN(function) =>
        new DescribeFunctionCmd(Option.empty, function)
      case _ =>
        throw new InterpreterException(s"Invalid syntax for DESCRIBE FUNCTION. " +
          s"""It should comply to the patterns: ${DESCRIBE_FUNCTION_WITH_KEYSPACE_PATTERN.toString}
              |or ${DESCRIBE_FUNCTION_PATTERN.toString}""".stripMargin)
    }
  }
  ...
  def extractDescribeAggregateCmd(text: String): DescribeAggregateCmd = {
    text match {
      case DESCRIBE_AGGREGATE_WITH_KEYSPACE_PATTERN(keyspace, aggregate) =>
        new DescribeAggregateCmd(Option(keyspace), aggregate)
      case DESCRIBE_AGGREGATE_PATTERN(aggregate) =>
        new DescribeAggregateCmd(Option.empty, aggregate)
      case _ =>
        throw new InterpreterException(s"Invalid syntax for DESCRIBE AGGREGATE. " +
          s"""It should comply to the patterns: ${DESCRIBE_AGGREGATE_WITH_KEYSPACE_PATTERN.toString}
              |or ${DESCRIBE_AGGREGATE_PATTERN.toString}""".stripMargin)
    }
  }
  ...
  def extractDescribeMaterializedViewCmd(text: String): DescribeMaterializedViewCmd = {
    text match {
      case DESCRIBE_MATERIALIZED_VIEW_WITH_KEYSPACE_PATTERN(keyspace, view) =>
        new DescribeMaterializedViewCmd(Option(keyspace), view)
      case DESCRIBE_MATERIALIZED_VIEW_PATTERN(view) =>
        new DescribeMaterializedViewCmd(Option.empty, view)
      case _ => throw new InterpreterException(s"Invalid syntax for DESCRIBE MATERIALIZED VIEW. " +
        s"""It should comply to the patterns: ${DESCRIBE_MATERIALIZED_VIEW_WITH_KEYSPACE_PATTERN.toString}
            | or ${DESCRIBE_MATERIALIZED_VIEW_PATTERN.toString}""".stripMargin)
    }
  }
