...
describe('Notebook benchmark contract', () => {
  ...
  it('distinguishes nearest-rank p95 from the max and from interpolation at n=20', () => {
    const samples: Sample[] = (['cold', 'warm'] as const).flatMap(cache =>
      Array.from({ length: 20 }, (_, index) => ({
        cache,
        run: index + 1,
        notebookReadyMs: (index + 1) * 10,
        fcpMs: index + 1
      }))
    );
    expect(summarize(samples, 20).cold).toEqual({
      count: 20,
      notebookReadyMs: { median: 105, p95: 190 },
      fcpMs: { median: 10.5, p95: 19 }
    });
  });
  ...
  it('rejects an old served index even when both builds remain available, while allowing server-injected markup', () => {
    ...
    expect(() => validateServedIndex(root, current.replace(/\.new\.js/g, '.old.js'))).toThrow(
