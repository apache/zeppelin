...
export const normalizeBundlePath = (path: string): string => path.replace(/\\/g, '/');
...
const angularEntryScripts = (index: string): string[] => {
  ...
  if (!scripts.length || !scripts.some(script => /^main[.-].*\.js$/.test(script))) {
    throw new Error(
      'Build output must contain a hashed Angular main entry script; run the production build (npm run build)'
    );
  }
