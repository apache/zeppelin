...
import { expect, test, BrowserContextOptions, Page } from '@playwright/test';
...
test.describe('Notebook route performance contract', () => {
  ...
  let importedNoteId: string | undefined;

  test.afterEach(async ({ request }) => {
    const noteId = importedNoteId;
    ...
    try {
      ...
      if (!deleted.ok()) {
        console.warn(`Benchmark note ${noteId} was not deleted (${deleted.status()}): ${await deleted.text()}`);
      }
    } catch (error) {
      console.warn(`Benchmark note ${noteId} was not deleted: ${error}`);
    }
  });
  ...
  test('records every cold and warm sample with readiness, FCP and build metadata', async ({
    ...
    const { use } = testInfo.project;
    const contextOptions: BrowserContextOptions = {
      ...Object.fromEntries(inheritedContextOptions.map(key => [key, use[key]])),
      baseURL,
      // browser.newContext resolves a relative storage state path against the process cwd, not the config.
      storageState: typeof use.storageState === 'string' ? resolve(root, use.storageState) : use.storageState,
    ...
    await test.step('Then raw samples and median/p95 retain every measurement', async () => {
      ...
      writeFileSync(resolve(output, `${runId}.summary.json`), `${JSON.stringify({ ...metadata, summary }, null, 2)}\n`);
