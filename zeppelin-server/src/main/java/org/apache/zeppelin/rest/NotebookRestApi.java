...
public class NotebookRestApi extends AbstractRestApi {
  ...
  @Path("reload/{noteId}")
  @ZeppelinApi
  public Response reloadNote(@PathParam("noteId") String noteId) throws IOException {
    Note note =
            notebookService.getNote(noteId, true, getServiceContext(), new RestServiceCallback());
    return new JsonResponse<>(Status.OK, "", note).build();
  }
