...
public class AppendOutputRunner {
  ...
  private boolean sameOutput(AppendOutputBuffer first, AppendOutputBuffer second) {
    return first.getIndex() == second.getIndex()
        ...
        && Objects.equals(first.getExecutionOwner(), second.getExecutionOwner());
  }

  private long flush(AppendOutputBuffer append, StringBuilder data) {
    long size = data.length();
    try {
      listener.onParagraphOutputAppend(append.getNoteId(), append.getParagraphId(),
          append.getIndex(), append.getExecutionOwner(), data.toString());
